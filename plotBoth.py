...
for i in range(3):
    df_a = pd.read_csv("bin/fcc_a{}.txt".format(i), names = names, header=0, sep = '\t')
    df_a.Order_Parameter = df_a.Order_Parameter.apply(lambda x : x if x < 1 else 1)
    if( i ==0):
         xp = ax1.twinx()
         xp.plot(df_a[names[2]],  markers[i], label ="T= {}".format(temp[i]), lw = 4)
         xp.set_ylabel(names[2] + " for T = {}".format(temp[i]), fontsize = 16, color = 'r')
         for tl in xp.get_yticklabels():
             tl.set_color('r')
             tl.set_fontsize(12)
    else:
        ax1.plot(df_a[names[2]],  markers[i], label ="T= {}".format(temp[i]), lw = 4)
        ax1.set_ylabel(names[2] + " for T = {} and T ={}".format(temp[1], temp[2]), fontsize = 16, color = 'k')
        for t in ax1.get_yticklabels():
             #t.set_color('r')
             t.set_fontsize(12)

    ax2.plot(df_a[names[3]],  markers[i], label ="T= {}".format(temp[i]), lw = 2, markersize = 1)
    ax3.plot(df_a[names[4]], markers[i], label ="T= {}".format(temp[i]), lw =2)
    ax4.plot(df_a[names[5]], markers[i], label ="T= {}".format(temp[i]), lw = 2)
